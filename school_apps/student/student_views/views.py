...
from school_apps.student.resources import StudentResource
...
def add_student(request):
    ...
    if request.method == 'POST':
        ...
        # )
            ...
            join_year =  student_form.cleaned_data["join_year"]
            user.student.join_year =  join_year
            
            bach = CourseCategory.objects.get(course_name="Bachelor")
            mast = CourseCategory.objects.get(course_name="Master")

            if request.user.adminuser.course_category == bach:
                
                semester_obj, created = Semester.objects.get_or_create(
                    course_category = bach,
                    bachelor_semester = join_year
                )
                user.student.semester = semester_obj
            
            if request.user.adminuser.course_category == mast:
                
                semester_obj, created = Semester.objects.get_or_create(
                    course_category = mast,
                    master_semester = join_year
                )
                user.student.semester = semester_obj
...
def student_file_upload(request):
    ...
    for column in file_data:
        ...
        dob_es_parse=""
        if (column[23]!="" ):
            dob_es_parse = parse(column[23]).date() #this change from 8/10/2020 to 2020-8-10 i.e in python format
        
        dob_bs_parse=""
        if (column[24]!="" ):
            dob_bs_parse = parse(column[24]).date()
        blood_group = column[25]
        # optional_subject = column[26]
        see_gpa = column[27]
        previous_school =  column[28]  
        # dob_es_parse = datetime.datetime.strptime(dob_es, "%Y-%m-%d")
    
        
        home_phone = home_phone,
        # home_phone[3:len-3] 
        father_name=column[15],
        father_phone=column[16],
        # home_phone[3:len-3] 
        mother_name=column[17],
        mother_phone=column[18],
        local_guardian_name=column[19],
        local_guardian_phone=column[20]
