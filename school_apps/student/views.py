...
from school_apps.courses.models import Exams, application_form, studentgrades, Subject, selectedcourses, Term
...
def postGrades(request):
    ...
    if term.is_published:
        grades_data =  studentgrades.objects.filter(Q(application_id__student_id=student)&Q(application_id__term=term)&~Q(marks=-1))
        remaining = studentgrades.objects.filter(Q(application_id__student_id=student)&Q(application_id__term=term)&Q(marks=-1))
        context = {'records':grades_data, 'remaining':remaining,'is_published':is_published}
...
def printapplicationform(request):
    ...
    try:
        # application=application_form.objects.get(Q(term=latest_term)&Q(student=student)&Q(status=True))
        application=application_form.objects.get(Q(term=latest_term)&Q(student=student)&Q(status=False))
        grades = studentgrades.objects.filter(application_id=application)
        context={'student':student, 'form':application, 'grades':grades}
        print("True")
...
def parent_checkscore(request):
    ...
    if request.method == "GET":
        ...
        application = application_form.objects.get(Q(student=Student.objects.get(student_user__username = request.POST['student'])) & Q(term=Term.objects.get(pk=request.POST['term'])))
        # grades = studentgrades.objects.filter(application_id=application)

        grades_data =  studentgrades.objects.filter(Q(application_id=application) &~Q(marks=-1))
        remaining = studentgrades.objects.filter(Q(application_id=application) &Q(marks=-1))
...
#     return render(request, "students/student_ajax_autocomplete.html", context)
