...
def submitscores(request):
    ...
    for exam in exams:
        ...
        test = studentgrades.objects.all().filter(application_id__student=student_object, exam_id= exam_object)
        if test.exists():
...
def bulkprintresults(request):
    ...
    if request.method == 'POST':
        applications  = application_form.objects.filter(term=request.POST['term_id'])

    context = {'applications':applications, 'terms':terms}
...
def examreport(request):
    ...
    if request.method=='GET':
        ...
        results = studentgrades.objects.all().filter(exam_id = exam).order_by('-marks')
        print(results)
        context = {
            'terms':terms,
            'results':results, 
...
def massexamapplication(request):
    ...
    if request.method == 'POST':
        ...
        students = Student.objects.filter(section=selected_section, faculty = request.POST['group'])
        ...
        for item in students:
            ...
            for sub_item in selected_subjects:
                try:
                    selected_exams.append(Exams.objects.get(term=selected_term, subject_id=sub_item.subject_id))
                except:
        ...
        forms = application_form.objects.filter(term=selected_term, student__faculty=request.POST['group'], student__section = selected_section)
        print(forms)
