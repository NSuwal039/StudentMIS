...
from .forms import (SubjectForm,SemesterForm,BachelorSemesterForm,MasterSemesterForm,SectionForm,
...
def manage_subject(request):
    ...
    print(request.user.adminuser, '\n')
    print(request.user.adminuser.course_category, '\n')
...
def view_student_syllabus(request):
    syllabus = Syllabus.objects.filter(semester = request.user.student.semester).first()
...
def add_assignment(request):
    ...
    if request.method == 'POST':
        ...
        #         student_bulk_email.append(email)
        #     context = {
        #     'message': "Please, Check Your Assignment in Mis.",
        # }
        #     template = get_template(
        #     'bulk_email/email_message.html').render(context)
