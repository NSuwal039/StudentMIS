...
class AssetsListView(PermissionRequiredMixin,ListView):
   ...
   def get(self, request, *args, **kwargs):
        ...
        for item in set(inventory_assets):
            item_list = Item.objects.filter(item_type=Assets.objects.get(pk=item))
...
def assignment_list(request, pk):
    ...
    return render(request, 'inventory/assignment_list.html', {'assets':assignments,'item':item, 'count':assignments_count})
...
def assets_department_assignment(request):
    ...
    if request.method=='GET':   
        return render(request,'inventory/assets_assignment.html', context=context)
    else:
        ...
        assets_transfer.objects.create(from_dept=Department.objects.none(), to_dept=department,item=item,overseer=request.user)
        
        msg_text= "{item} assigned to {department}."
        messages.success(request, msg_text.format(
            item=item,
...
def assets_return(request):
    ...
    messages.success(request, msg_string.format(
        item=item
    ))
    return HttpResponseRedirect(reverse('inventory:inventory_list'))
...
def get_items_details_ajax(request):
    ...
    items = Item.objects.filter(Q(item_type=item_type), Q(current_department=None), Q(current_branch=None))
...
#     try:
#         amount = assets_assignment.objects.get(department=department,item=item).amount
#     except:
#         amount = 0
#     # print('amount:',amount)
...
def assets_dept_transfer(request):
    ...
    if request.method =='GET':
        return render(request,'inventory/assets_transfer.html', context=context)
    else:
        ...
        try:
            print('try1')
            assets_transfer.objects.create(from_dept=from_dept, to_dept=to_dept,item=item,overseer=request.user)
        except:
            try:
                print('try2')
                assets_transfer.objects.create(from_dept=from_dept, to_branch=to_branch,item=item,overseer=request.user)
            except:
                try:
                    print('try3')
                    assets_transfer.objects.create(from_branch=from_branch, to_branch=to_branch,item=item,overseer=request.user)
                except:
                    try:
                        print('try4')
                        assets_transfer.objects.create(from_branch=from_branch, to_dept=to_dept,item=item,overseer=request.user)
                    except:
...
def get_dept_assets_ajax(request):
    ...
    return render(request,'ajax/dept_item_list.html', context={'items':items})
...
class ProcurementRequestListView(PermissionRequiredMixin, ListView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(request, 'procurement/procurement_list.html', context=context)
...
class CreateProcurementRequest(PermissionRequiredMixin, CreateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        context={
            'form':ProcurementRequestCreateForm
        }
        return render(request, 'procurement/create_procurement_request.html', context=context) 
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(reverse('inventory:create_procurement_request'))
        else:
...
class TransactionCreateView(PermissionRequiredMixin, CreateView):
    ...
    def get(self, request, *args,**kwargs):
        ...
        return render(request, 'transaction/create_transaction_record.html', context=context)
    ...
    def post(self, request, *args , **kwargs):
        ...
        if form.is_valid():
            ...
            for i in range(0, amount):
                transaction_items.objects.create(transaction=transaction,
                                                item_type=item_type,
                                                )
            ...
            messages.error(request, list(form.errors))
            return HttpResponseRedirect(reverse('inventory:create_transaction'))
...
class TransactionListView(PermissionRequiredMixin, ListView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        messages.success(request, 'Transaction completed. Please registered newly acquired items to inventory.')
        return render(request, 'transaction/transaction_list.html', context)

def register_new_inventory_ajax(request):
    transaction = Transaction.objects.get(pk=request.GET['id'])
    items_transaction = transaction_items.objects.filter(transaction=transaction)
    department = Department.objects.all()
    branch = Branch.objects.all()
    ...
    return render(request, 'transaction/register_item_modal_body.html', context)
...
def transaction_register_item(request):
    ...
    if destination == 'department':
        ...
        to_create = Item.objects.create(id_no=item_id, item_type=new_item.item_type, category=new_item.item_type.category,
                                        current_department=department, source_transaction=new_item.transaction)
    elif destination == 'branch':
        branch = Branch.objects.get(pk=request.GET['dept_id'])
        to_create = Item.objects.create(id_no=item_id, item_type=new_item.item_type, category=new_item.item_type.category,
                                        current_branch=branch, source_transaction=new_item.transaction)
    else:
        to_create = Item.objects.create(id_no=item_id, item_type=new_item.item_type, category=new_item.item_type.category,
